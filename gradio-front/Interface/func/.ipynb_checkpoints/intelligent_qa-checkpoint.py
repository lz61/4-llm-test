...
def read_csv_data(csv_file_path, user_id):
    df = pd.read_csv(csv_file_path, names=['user_id', 'article_id', 'time', 'operation'])
    # 读取特定用户的数据
...
def search(index, query, corpus, topk=3):
    ...
    scores, ids = index.search( q_embs.reshape(1, -1), topk )  # scores[]列表中，每个元素表示一个query与语料库中所有文本块的相似度分数列表
    candidate_docs = [corpus[int(i)] for i in ids[0]]  # 注意：目前只针对单个query，所以用 ids[0]
    ranked_docs = sorted(zip(scores[0], candidate_docs), key=lambda x: x[0], reverse=True)
...
def intelligent_qa_func(article_ids, user_query):
    ...
    # 生成本地知识库 embedding
    index, descriptions, titles, links = build_index(json_file_path, article_ids, batch_size)    
    ...
    # 调用问答模型
    output_text = response_qa(query, ranked_docs)
    ...
    #     f"\n接入本地知识库后的回答: {result_with_prompt}"
    # )
    ...
    for _, (_, sentence) in enumerate(ranked_docs):
